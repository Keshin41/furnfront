...
import { Eye, FileText, List, PlusCircle, Users } from "lucide-vue-next";
...
const data = {
  totalFurMeets: 42,
  publishedFurMeets: 30,
  draftFurMeets: 12,
  recentFurMeets: [
    {
      id: 1,
      title: "FurMeet de Paris",
      date: "2024-05-15",
      is_published: true,
    },
    {
      id: 2,
      title: "FurMeet de Lyon",
      date: "2024-06-20",
      is_published: false,
    },
    {
      id: 3,
      title: "FurMeet de Marseille",
      date: "2024-07-10",
      is_published: true,
    },
  ],
    ...
    <div class="container mx-auto px-4">
      ...
      <div class="mb-8">
        ...
        <p class="text-gray-400">
          Bienvenue sur l'interface d'administration FurTours
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <!-- Total FurMeets -->
        <div
          class="rounded-lg border border-light-blue/30 bg-linear-to-br from-light-blue/20 to-blue/10 p-6"
        >
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-sm font-medium text-gray-400">Total FurMeets</h3>
            <FileText class="h-5 w-5 text-light-blue" />
          </div>
          <p class="text-4xl font-bold text-white">{{ data.totalFurMeets }}</p>
        </div>

        <!-- Published -->
        <div
          class="rounded-lg border border-green-500/30 bg-linear-to-br from-green-500/20 to-green-600/10 p-6"
        >
          <div class="flex items-center justify-between mb-2">
          ...
          <p class="text-4xl font-bold text-white">
            {{ data.publishedFurMeets }}
          </p>
        </div>

        <!-- Drafts -->
        <div
          class="rounded-lg border border-yellow-500/30 bg-linear-to-br from-yellow-500/20 to-yellow-600/10 p-6"
        >
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-sm font-medium text-gray-400">Brouillons</h3>
            <FileText class="h-5 w-5 text-yellow-400" />
          </div>
          <p class="text-4xl font-bold text-white">{{ data.draftFurMeets }}</p>
        </div>
      </div>
      ...
      <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <UButton
            ...
            class="bg-linear-to-r from-light-blue to-cyan-500 px-6 py-8 font-semibold text-[#2c3e50] hover:from-cyan-300 hover:to-light-blue hover:shadow-lg hover:shadow-light-blue/30 text-lg h-auto"
          >
      ...
      <!-- Recent FurMeets -->
      <div>
        <h2 class="text-2xl font-bold text-light-blue mb-4">
          FurMeets récents
        </h2>
        <div
          v-if="data.recentFurMeets.length > 0"
          class="rounded-lg border border-light-blue/30 bg-blue/20 overflow-hidden"
        >
          <div class="divide-y divide-gray-700">
            <div
              v-for="furMeet in data.recentFurMeets"
              :key="furMeet.id"
