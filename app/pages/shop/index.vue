  ...
  <div class="min-h-screen bg-brand-white text-slate-900">
    ...
    <main class="mx-auto min-h-screen max-w-6xl px-6 py-12">
      <section class="mb-10 text-center">
        <h1 class="text-4xl font-bold text-brand-dark-blue">Boutique Fur'N'Tours</h1>
        <p class="mt-3 text-gray-600">Découvrez nos produits officiels et nos goodies pour soutenir l’association.</p>
      </section>
      ...
      <section>
        ...
        <div v-else>
          <div v-if="products.length === 0" class="text-center text-slate-500 py-16">
          ...
          <div v-else class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            <div v-for="product in products" :key="product.id" class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              ...
              <div class="p-4">
                ...
                <p class="mt-2 text-sm text-slate-500 truncate">{{ product.description || 'Aucune description disponible.' }}</p>
                ...
                <div class="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                  <span v-if="product.category" class="px-2 py-1 bg-slate-100 rounded-full">{{ product.category }}</span>
                  <span v-if="product.virtual" class="px-2 py-1 bg-green-100 text-green-700 rounded-full">Produit virtuel</span>
                </div>
              </div>
            </div>
          </div>
...
const { data, error, pending } = await useAPI<Product[]>('/product?virtual=false');

const products = computed(() => data.value ?? []);
const isLoading = computed(() => pending.value);

const displayMinPrice = (product: Product): string => {
  const skuPrices = product.skus.map((sku) => Number(sku.priceOverride ?? product.basePrice));
