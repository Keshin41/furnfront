<template>
  <div class="min-h-screen bg-brand-white text-slate-900">
    ...
    <main class="mx-auto min-h-screen max-w-4xl px-6 py-12">
      ...
      <div v-if="loading" class="py-24 text-center text-slate-400">Chargement...</div>
      <div v-else-if="error" class="py-24 text-center text-red-500">Erreur : {{ error.message || error }}</div>
      <div v-else-if="!product" class="py-24 text-center text-slate-500">Produit introuvable</div>
      <div v-else class="grid gap-8 lg:grid-cols-[1fr_1fr]">
        <div class="flex rounded-3xl border border-slate-100 bg-slate-50 items-center justify-center p-6 min-h-80">
          <ImageWithFallback
        ...
        <div>
          ...
          <div v-if="product.optionTypes.length" class="mt-6 space-y-5">
            <div v-for="optionType in product.optionTypes" :key="optionType.id">
              <div class="mb-2 flex items-center justify-between gap-3">
                <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-500">{{ optionType.name }}</h2>
                <span class="text-sm text-slate-500">
                  {{ optionType.optionValues.find((value) => value.id === selectedOptions[optionType.id])?.value }}
                </span>
              </div>
              ...
              <div class="flex flex-wrap gap-2">
                <button
                  ...
                  :class="selectedOptions[optionType.id] === optionValue.id
          ...
          <div class="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-4">
            <div v-if="selectedSku" class="space-y-2">
              <div v-if="selectedOptionSummary" class="flex items-center justify-between gap-3">
                <span class="text-sm font-medium text-slate-500">Variante</span>
                <span class="text-right text-slate-900">{{ selectedOptionSummary }}</span>
              </div>
              <div class="flex items-center justify-between gap-3">
                <span class="text-sm font-medium text-slate-500">Stock</span>
                <span class="text-slate-900">
                  {{ selectedSku.trackStock ? `${selectedSku.stock} disponible(s)` : 'Stock non suivi' }}
                </span>
              </div>
            </div>
            <p v-else class="text-sm text-red-500">Cette combinaison n'est pas disponible.</p>
          </div>
...
watch(
  ...
  (value) => {
    ...
    selectedOptions.value = Object.fromEntries(
      value.optionTypes.map((optionType) => [optionType.id, optionType.optionValues[0]?.id ?? ''])
    );
  },
  { immediate: true }
);
...
const selectedImageUrl = computed(() => selectedSku.value?.imageUrl || product.value?.imageUrl || null);
...
const selectedPrice = computed(() => {
  ...
  return Number(selectedSku.value?.priceOverride ?? product.value.basePrice).toFixed(2);
});
...
const selectedOptionSummary = computed(() => {
  ...
  return selectedSku.value.options.map((option) => option.optionValue.value).join(' / ');
});
