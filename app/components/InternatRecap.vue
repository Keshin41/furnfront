...
const total = props.basket.reduce((previousValue, currentValue) => {
  return previousValue + (Number.parseFloat(currentValue.unitPrice) * currentValue.quantity);
}, 0);
        ...
        >
          <div class="flex flex-col gap-1">
            ...
            <p class="text-xs text-neutral-600">
              {{ item.quantity }} × {{ Number.parseFloat(item.unitPrice).toFixed(2) }} €
            </p>
          </div>
