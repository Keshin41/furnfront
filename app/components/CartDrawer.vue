  ...
  <USlideover v-model:open="open" side="right">
    <template #content>
      <div class="flex h-full flex-col">
        ...
        <div v-if="items.length === 0" class="flex flex-1 flex-col items-center justify-center gap-3 text-slate-400">
          <UIcon name="i-lucide-shopping-cart" class="h-12 w-12 opacity-30" />
          <p class="text-sm">Ton panier est vide.</p>
        </div>

        <ul v-else class="flex-1 overflow-y-auto divide-y divide-slate-100 px-5">
          <li v-for="item in items" :key="item.skuId" class="flex gap-4 py-4">
            ...
            <div class="flex flex-1 flex-col gap-1">
              <span class="font-semibold text-brand-dark-blue leading-tight">{{ item.productName }}</span>
              <span v-if="item.variantLabel" class="text-xs text-slate-400">{{ item.variantLabel }}</span>
              <span class="text-sm font-bold text-brand-blue">{{ (item.price * item.quantity).toFixed(2) }} €</span>
        ...
        <div v-if="items.length > 0" class="border-t border-slate-100 px-5 py-4 space-y-3">
          ...
          <UButton block label="Commander" color="primary" size="lg" />
          <UButton
            block
